...
void Field::ComputeCornerE(AthenaArray<Real> &w, AthenaArray<Real> &bcc) {
  ...
  AthenaArray<Real> &evel1 = ex->vv[(IVX-1)]; // need the volume velocities here
  AthenaArray<Real> &evel2 = ex->vv[(IVY-1)];
  AthenaArray<Real> &evel3 = ex->vv[(IVZ-1)];
...
//---- 3-D update:
    ...
    for (int k=ks-1; k<=ke+1; ++k) {
      if (EXPANDING_ENABLED) ev3 = evel3(k);
      for (int j=js-1; j<=je+1; ++j) {
        if (EXPANDING_ENABLED) ev2 = evel2(j);
#if GENERAL_RELATIVITY==1
...
#pragma omp simd
          for (int i=is; i<=ie; ++i) {
            cc_e_(k,j,i) =   (w(IVZ,k,j,i)-ev3)*bcc(IB2,k,j,i) 
                           - (w(IVY,k,j,i)-ev2)*bcc(IB3,k,j,i);
          }
        } else {
#pragma omp simd
          for (int i=is; i<=ie; ++i) {
...
#pragma omp simd
          for (int i=is-1; i<=ie+1; ++i) {
            ev1          = evel1(i);
            cc_e_(k,j,i) =   (w(IVX,k,j,i)-ev1)*bcc(IB3,k,j,i) 
                           - (w(IVZ,k,j,i)-ev3)*bcc(IB1,k,j,i);
          }
        } else {
#pragma omp simd
          for (int i=is-1; i<=ie+1; ++i) {
...
#pragma omp simd
  ...
  // not needed if we can just add vg. 
  //if (EXPANDING_ENABLED) 
